...
//long double R = A;              // representative value for Sun-Earth distance
...
//if(argc!=5){
 if(argc<7){
  printf("Usage: e.g. ./a.out cLoverkX EovermcSquX lambdaX Ngrid Rtau vmass\n");
  return 1;
//exit(1);
 }
 ...
 long double cLoverkX    =atof(argv[1]);
 long double EovermcSquX =atof(argv[2]);
 long double lambdaX     =atof(argv[3]);
 long double Ngrid       =atof(argv[4]);
 long double Rtau        =atof(argv[5]);
 long double vmass       =atof(argv[6]);
...
//long double T = C/V;
 ...
 printf("                    kappaSqu  %+25.15Le         typical Earth kappa squared\n",kappaSqu); 

 long double kappa = sqrt(kappaSqu);
 printf("                    kappa     %+25.15Le         typical Earth kappa\n\n",kappa); 

 long double lambda = (kappa*L*c)*(kappa*L*c)/k/E;
 printf("                    lambda    %+25.15Le         typical Earth lambda\n",lambda); 

 long double h0 = c*sqrt( EovermcSqu - kappaSqu );
 printf("                    h0        %+25.15Le         typical Earth h0\n\n",h0); 

 long double epsilon = Lcoverk*sqrt( 1.L-(kappa/EovermcSqu)*(kappa/EovermcSqu) );
 printf("                    epsilon   %+25.15Le         typical Earth epsilon\n\n",epsilon); 
...
//long double T=0.;
 ...
 long double koverLcX   = 1.L/cLoverkX;
 long double kappaSquX  = 1.L-koverLcX*koverLcX;        // Munoz Eq. (11)
 long double kappaX     = sqrt(kappaSquX);
 printf("                    kappaX    %+25.15Le                 kappaX\n\n",kappaX); 
 long double mcSquoverE = 1.L/EovermcSquX;
 long double epsilonX   = cLoverkX*sqrt(1.L-mcSquoverE*mcSquoverE*kappaSquX);
 printf("                    epsilonX  %+25.15Le                epsilonX\n\n",epsilonX); 

 FILE *fptr2;
 fptr2 = fopen("special-relativistic-Kepler.out", "w");
 long double tau2=Rtau*PI;
 for(Thta=0.;Thta<tau2;Thta+=delThta){
  ThtM=Thta+delThtH;
  r=lambdaX/( 1.L+epsilonX*cos(kappaX*ThtM) );
  x=r*cos(Thta);
  y=r*sin(Thta);
  fprintf(fptr2, "%+25.15Le, %+25.15Le\n",x,y);
 }
 fclose(fptr2);
