...
//shell.readSXF(Args() << Arg("file",  "./cosyRFS.sxf"));
  shell.readSXF(Args() << Arg("file",  "./COSY.sxf"));
...
//shell.writeSXF(Args() << Arg("file",  outputFile.c_str()));
  ...
  std::string analysisFile = "./cosy.analysis";

  shell.analysis(Args());

  // Calculate a linear matrix

  std::string mapFile = "./cosy.map1";
  ...
  std::string twissFile = "./cosy.twiss";
