...
bool COSY::Shell::use(const UAL::Arguments& arguments) {
...
void COSY::Shell::addN(const std::string& name, double value)
{
...
void COSY::Shell::addMadK1K2(const std::string& name, double k1, double k2)
{
...
void COSY::Shell::calculateTwiss() {
    if (p_lattice == 0) {
        std::cerr << "There is no " + m_accName << " accelerator " << endl;
        return;
    }
...
void COSY::Shell::writeTwissToFile(const char* fileName) {
...
void COSY::Shell::calculateMaps(PacVTps& oneTurnMap,
        std::vector<PacVTps>& maps,
        int order,
...
void COSY::Shell::calculateTwiss(const PacVTps& oneTurnMap,
