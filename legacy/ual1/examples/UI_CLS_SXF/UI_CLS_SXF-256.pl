...
$bunch->setBeamAttributes(3.0, 0.00051099895069);        # This is for 3.0 GeV electron tracking

for($i =0; $i < $size; $i++){$bunch->setPosition($i, 1.e-2*($i+1), 0.0, 1.e-2*($i+1), 0.0, 0.0, 1.e-3*($i+1));
}
# $shell->run("turns" => 256, "step" => 1, "print" => "./out/" . "/test/" . "/fort.8" ,  "bunch" => $bunch);
$shell->run("turns" => 256, "step" => 1, "print" => "./out/" . "/fort.8RAW" ,  "bunch" => $bunch);

# ------------------------------------------------------------------------
# Copy only the last 256 turns of out/fort.8RAW to out/test/fort.8
# ------------------------------------------------------------------------

print "Copy only the last 256 turns of out/fort.8RAW to out/test/fort.8', p. 320 of Perl Cookbook---NOT", "\n";
system 'tail -256 "out/fort.8RAW" > "out/test/fort.8" ';
