...
//       $Id: XrdSectestServer.cc,v 1.5 2005/03/01 05:58:46 abh Exp $

const char *XrdSectestServerCVSID = "$Id: XrdSectestServer.cc,v 1.5 2005/03/01 05:58:46 abh Exp $";
   ...
   if (opts.host)
      {struct hostent *hp;
       if (!(hp = gethostbyname(opts.host)))
          {std::cerr <<"testServer: host '" <<opts.host <<"' not found." <<std::endl;
           exit(1);
          }
