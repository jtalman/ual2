...
//        $Id: XrdXrootdXeqAio.cc,v 1.2 2005/01/16 04:37:48 abh Exp $

const char *XrdXrootdAiocbCVSID = "$Id: XrdXrootdXeqAio.cc,v 1.2 2005/01/16 04:37:48 abh Exp $";
...
/*
   while (myIOLen > 0)
       {if (!(aiop = myAioReq->getAio()))
             {Resume = &XrdXrootdProtocol::aio_WriteAll;
              myBlen = 0;
              return -EINPROGRESS;
             }

     Quantum = (aiop->buffp->bsize > myIOLen ? myIOLen
                                                  : aiop->buffp->bsize);
