...
//         $Id: XrdXrMain.cc,v 1.9 2005/03/01 05:59:07 abh Exp $

const char *XrdXrMainCVSID = "$Id: XrdXrMain.cc,v 1.9 2005/03/01 05:59:07 abh Exp $";
...
void printHelp() 
{
  std::cout << "XrClient help: " << std::endl;
  std::cout << "auth <credtype> <cred>" << std::endl;
  std::cout << "close          close the currently open file" << std::endl;
  std::cout << "exit           exit program" << std::endl;
  std::cout << "login [<username>] [<role>]" << std::endl;
  std::cout << "    <username> is the Unix UID that is used to login " << std::endl;
  std::cout << "    <role>     can be either kXR_useruser (default) or kXR_useradmin" << std::endl;
  std::cout << "open <path> [a|c|...] [<mode>]" << std::endl;
  std::cout << "    <path>     is the absolute file path on the remote host " << std::endl; 
  std::cout << "    a ...      open flags - see protocol specification for details " << std::endl;
  std::cout << "    <mode>     access permission mode - see protocol specification for details " << std::endl;
  std::cout << "outfile <filename> " << std::endl;
  std::cout << "               a read request will write to the file <filename>" << std::endl;
  std::cout << "quit           exit program" << std::endl;
  std::cout << "read <offset> <length>" << std::endl;
  std::cout << "stat <path>    status for a file identified by path" << std::endl;
  std::cout << "test           run a few test commands" << std::endl; 
...
void doOutfile(char* buffer)
{
  ...
  if (filename == "") {
    cerr << "Filepath is missing." << std::endl;
    return;
  }
  ...
  if (fd == -1) {
    if (errno == 13) {
      cerr << "Error: cannot create the file " << filename << std::endl;
      return;
    }
    cerr << "Error in creating file: " << errno << std::endl;
  } 
...
void doLogin(char *buffer)
{
  ...
  if (args == 0) {
	...
	cerr << "Incorrect role: " 
	     << " can only be kXR_useradmin or kXR_useruser. " << std::endl;
      } 
    }
  ...
  if (client->login((kXR_char*) username.c_str(), 
    ...
    cerr << "Error: login not done correctly. " << std::endl;
  }
...
void doAuth(char *buffer) 
{
  ...
  if (client->auth(credtype)) {
    cerr << "Error: authentication not done correctly. " << std::endl;
  } 
} // doAuth
...
void doOpen(char *buffer)
{
  ...
  if (filename == "") {
    cerr << "Filename required for open." << std::endl;
    return;
  }
  ...
  if (numberOfTokens(buffer) >= 3) {
    ...
    else                   {cerr << "Invalid open flag" << std::endl;}
  ...
  if (client->open((kXR_char*) filename.c_str(), 
		   (kXR_unt16) oflags, 
		   (kXR_unt16) mode              ) != 0) {
    cerr << "Error: file not opened correctly. " << std::endl;
  }
...

{
  ...
  if (offset == "") {
    cerr << "Offset required for read." << std::endl;
    return;
  } 
  ...
  if (len == "") {
    cerr << "Length (i.e. number of bytes) required for read." << std::endl;
    return;
  }
  ...
  if (rc < 0) {
    cerr << "Error: file not read correctly. " << std::endl;
  } else {

    // If an outfile was used, write the buffer into the file
    //
    if (fd > 0) {
      std::cout << write(fd, buf, rc) 
	   << " bytes have been written to the outfile." << std::endl;
    }
  }
...
void doStat(char *buffer) 
{
  ...
  if (client->stat(&buf, (kXR_char*) getToken(buffer, 1).c_str()) != 0) {
    cerr << "Error: file status not obtained correctly. " << std::endl;
  } else {
    std::cout << "id="   << buf.st_dev   << "," << buf.st_ino << ", "
	 << "size=" << buf.st_size  << ", "
	 << "mode=" << buf.st_mode  << ", "
	 << "stat=" << buf.st_mtime << std::endl;
  }
...
void doClose() 
{
  ...
  if (client->close() != 0) {
    cerr << "Error: file not closed correctly. " << std::endl;
  }
} // doClose
...
void doTest() 
{
  ...
  //
  std::cout << "Test open /tmp/heinz/testfile\n";
...
int parseInput(char *buffer) 
{
  ...
  //
  cerr << "Unknown command. Enter 'help' for help." << std::endl;
  return xr_error;
...
{ 
  ...
  //
  if (argc < 2) {
    cerr << "usage: xrclient [-d] hostname [port] " << std::endl;
    _exit(-1);
  } else {
  ...
  do {
    ...
    if (!cin.getline(input, len)) {std::cout <<std::endl; break;} // At eof
